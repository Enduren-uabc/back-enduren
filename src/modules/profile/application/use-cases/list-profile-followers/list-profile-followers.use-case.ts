...
export interface ListProfileFollowersInput {
  userId: string;
}

export class ListProfileFollowersUseCase {
  constructor(
  ...
  public async execute(
    _actor: CurrentActor,
    input: ListProfileFollowersInput,
  ): Promise<ProfileDto[]> {
    await this.ensureProfileExists(input.userId);

    const follows = await this.followRepository.findFollowersOf(input.userId);

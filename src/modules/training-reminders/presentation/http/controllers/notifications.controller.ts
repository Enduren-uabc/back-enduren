...
export class NotificationsController {
  ...
  public async sendTest(
    ...
    const body =
      'Si ves esto, el módulo de notificaciones funciona correctamente.';
    ...
    if (tokenValues.length > 0) {
      ...
      this.logger.log(
        `Test push sent to user ${user.sub} (${tokenValues.length} tokens)`,
      );
    } else {
      this.logger.warn(
        `No push tokens for user ${user.sub}. Push skipped, in-app saved.`,
      );
    }
